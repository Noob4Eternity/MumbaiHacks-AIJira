...
os.environ['GEMINI_API_KEY'] = os.getenv("GEMINI_API_KEY")  # Now it should have a value
model = LLM(
...
class TaskAutomationCrew:
    ...
    def __init__(self):
        ...
        self.agents_config = self.load_yaml('src/mumbaihacks/config/agents.yaml')
        self.tasks_config = self.load_yaml('src/mumbaihacks/config/tasks.yaml')
